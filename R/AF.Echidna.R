...
# @param trait.mod  the mode of trait value distribution, such as '!logit', etc. 	     
...
# @rdname  AF.Echidna
# @usage echidnaT(es0.file,trait,trait.mod,
                   ...
                   # qualifier,jobqualf)
#  @export
# echidnaT<-function(es0.file,trait=NULL,trait.mod=NULL,
#                    fixed=NULL,
...
#   
#   ## G-structure
#   #random=c(~Fam,~Fam+Plot)
#   #random=c(~Fam)
#   #random=~Fam
#   #random=list(random)
#   if(!is.null(random)){
#     if(class(random)!="list") random<-list(random)
#     random1<-list();length(random1)<-length(random)
#     if(length(random)>1) 
#       cat('\nProgram runs for 2 more G-structure at one time.\n')
#     
#     random1<-lapply(random, as.character)
#     #else random1[[1]]<-as.character(random)
#     
#     if(length(random1[[1]])==3){
#       ran0<-unlist(lapply(random1, function(x) x[2]))
#       ran1<-unlist(lapply(random1, function(x) x[3]))
...
#     
#     ttN2<-length(ran1)
#     if(is.na(ran0)) ran0<-paste0('G',1:ttN2)
#   } else {ttN2<-1;ran1<-' '}
#   
#   
#   ## R-structure
#   if(is.null(residual)) residual<-list(~units)
#   if(class(residual)!="list") residual<-list(residual)
#   residual1<-list();length(residual1)<-length(residual)
#   if(length(residual)>1) 
#     cat('\nProgram runs for 2 more R-structure at one time.\n')
#   
#   residual1<-lapply(residual, as.character)
#   #else residual1[[1]]<-as.character(residual)
#   
#   if(length(residual1[[1]])==3){
#     resid0<-unlist(lapply(residual1, function(x) x[2]))
#     resid1<-unlist(lapply(residual1, function(x) x[3]))
...
#   
#   ttN3<-length(resid1)
#   if(is.na(resid0)) resid0<-paste0('R',1:ttN3)
#   
#   #i=0;
#   ss<-lapply(ttN, function(x){
#     if(trace==TRUE & batch==TRUE) cat('\nrun trait',x,'-- -- --:')
...
#     }
#     
#     xxx<-lapply(1:ttN2,function(y){
#       # if(trace==TRUE & length(random)>1) 
#       # cat('\nrun ',paste0('G',y),'-- random effects:', ran1[y])
#       
#       xx<-lapply(1:ttN3,function(z){
#         #i=i+y+z-1
#         #if(trace==TRUE & length(residual)>1) 
#         cat('\nrun ',paste0('G',y),'-- random effects:', ran1[y])
#         cat('\nrun ',paste0('R',z),'-- residual effects:', resid1[z])
#         
#         # AFEchidna::
#         x<-run.mod(es0.file=es0.file,
#                    trait=x,trait.mod=trait.mod,
#                    fixed=fixed,random=ran1[y],residual=resid1[z],
#                    mulT=mulT,met=met,
#                    trace=trace,delf=delf,foldN=foldN,
#                    predict=predict,vpredict=vpredict,
#                    maxit=maxit,cycle=cycle,Fmv=Fmv,
#                    qualifier=qualifier,jobqualf=jobqualf)
#         
#         org.par<-list(es0.file=es0.file,
#                       trait=trait,trait.mod=trait.mod,
#                       fixed=fixed,random=random,residual=residual,
#                       mulT=mulT,mulN=mulN,mulp=mulp,
#                       met=met,trace=trace,delf=delf,
#                       Fmv=Fmv,cycle=cycle,batch=FALSE,
#                       #batch.G=batch.G,batch.R=batch.R,
#                       predict=predict,vpredict=vpredict,
#                       qualifier=qualifier,jobqualf=jobqualf)
#         
...
# 
# # @rdname  AF.Echidna
# # @usage echidna.batch(es0.file,trait,
# #                         fixed=NULL,random=NULL,residual=NULL,
# #                         mulT=FALSE,mulN=NULL,met=FALSE,
# #                         trace=TRUE,delf=TRUE,mulp=NULL,
# #                         predict=NULL,vpredict=NULL,
# #                         maxit=30,
# #                         qualifier=NULL,jobqualf=NULL)
# # @export
# echidna.batch<-function(es0.file,trait,
#                         fixed=NULL,random=NULL,residual=NULL,
#                         mulT=FALSE,mulN=NULL,met=FALSE,
#                         trace=TRUE,delf=TRUE,mulp=NULL,
#                         predict=NULL,vpredict=NULL,
...
#                         qualifier=NULL,jobqualf=NULL){
# 
#   require(dplyr,warn=F,quiet=T)
#   require(tidyr,warn=F,quiet=T) # unite
# 
#   cat('\nProgram starts running batch analysis ------ \n')
# 
#   if(trace==FALSE & !is.null(mulp)){
...
#         if(bbn==1) x=paste(bb,collapse=' ')
#       }
#     AFEchidna::echidna(es0.file=es0.file,trait=x,
#                        fixed=fixed,random=random,residual=residual,
#                        mulT=mulT,met=met,
#                        trace=trace,delf=delf,
#                        predict=predict,vpredict=vpredict,
#                        maxit=maxit,
#                        qualifier=qualifier,jobqualf=jobqualf)
#   })
# 
# 
#   if(mulT==F) names(ss)<-trait
#   if(mulT==T) names(ss)<-unlist(lapply(ss,function(x) x$Traits))
# 
#   NTrait<-names(ss)
#   NTrait<-gsub(' ','-',NTrait)
#   NTrait<-sub('-$','',NTrait)
...
#   tt<-tt1<-NULL
# 
#   tt$res.all<-ss#lapply(ss,function(x) x$esr)
# 
#   tt1$esr.all<-ss#lapply(ss,function(x) x$esr) ###!!!
#   tt1$evp.all<-lapply(ss,function(x) x$evp)
#   tt1$vpc.all<-lapply(ss,function(x) x$vpc)
#   tt1$vpv.all<-lapply(ss,function(x) x$vpv)
#   # tt$ess.all<-lapply(ss,function(x) x$ess)
#   # tt$coef.all<-lapply(ss,function(x) x$coef)
#   Converge<-sapply(tt1$esr.all,function(x) x$Converge)
#   maxit<-sapply(tt1$esr.all,function(x) x$maxit)
# 
#   class(tt1)<-'esR'
#   if(!is.null(mulp)) {
#     pin.tt1<-AFEchidna::pin(tt1,mulp,Rres=T)
#     pin.tt1$res$Converge<-Converge
#     pin.tt1$res$maxit<-maxit
# 
#     ## vcms !!!
#     aa<-pin.tt1$vcms
#     aa1<-strsplit(aa,'; ')[[1]]
...
#       #vcms<-paste(aa1,collapse='; ')
#       #names(pin.tt1$res)[1:length(aa1)]<-paste0('V',1:length(aa1))
#       aa1<-AFEchidna::vcms.fun(aa1,mulN)
#     }
#     aa1<-paste(paste0('V',1:length(aa1)),aa1,sep='-')
#     vcms<-paste(aa1,collapse='; ')
# 
...
#     #var<-NULL
#     var<-list()
#     length(var)<-length(NTrait)
#     if(!is.null(tt1$vpc.all)){ # !cycle, batch
#          for(i in 1:length(NTrait)) {
#            var0<-AFEchidna::Var(tt1$esr.all[[i]])
#            terms<-var0[,'Term']
...
#            var[[i]]<-mat.var
#          }
#       # var<-lapply(1:length(trait), function(x){
#       #   var0<-AFEchidna::Var(tt1$esr.all[[x]])
#       #   terms<-var0[,'Term']
#       #   mat.var<-matrix(c(var0[,'Sigma'],var0[,'SE']),nrow=1)
#       #   mat.var<-as.data.frame(mat.var)
#       #   names(mat.var)<-c(paste0('V',1:nrow(var0)),paste0('SE',1:nrow(var0)))
#       #   mat.var
#       # })
# 
#     }
#     var1<-do.call(rbind,var)
#     row.names(var1)<-NTrait#names(ss)
#     var1$Converge<-Converge
#     var1$maxit<-maxit
# 
#     if(mulT) terms<-AFEchidna::vcms.fun(terms,mulN)
# 
#     terms<-paste(paste0('V',1:length(terms)),terms,sep='-',collapse='; ')
# 
#     if(trace==FALSE){
#       cat('\nterms: ',terms,'\n')
#       print.data.frame(var1)
#       cat('\n')
#     }
# 
#     attr(var1, "heading") <- paste0('\n',terms,'\n',
...
#   }
# 
#   org.par<-list(es0.file=es0.file,trait=trait,
#                 fixed=fixed,random=random,residual=residual,
#                 mulT=mulT,mulN=mulN,met=met,
#                 trace=trace,delf=delf,mulp=mulp,
#                 predict=predict,vpredict=vpredict,
#                 qualifier=qualifier,jobqualf=jobqualf)
# 
#   tt$org.par<-org.par
...
# # @export  update.esRb
# # @export
# update.esRb<-function(object,trait=NULL,fixed=NULL,
#                            random=NULL,residual=NULL,
#                            mulp=NULL,
#                            predict=NULL,vpredict=NULL,
#                            maxit=30,
#                            qualifier=NULL,jobqualf=NULL,...){
#   # object$org.par$batch==T ##??
# 
#   #object<-res21
#   org.par<-object$org.par
# 
...
#   if(is.null(random)) random<-org.par$random
#   if(is.null(residual)) residual<-org.par$residual
#   if(is.null(mulp)) mulp<-org.par$mulp
# 
#   if(is.null(predict)) predict<-org.par$predict
#   if(is.null(vpredict)) vpredict<-org.par$vpredict
#   if(is.null(qualifier)) qualifier<-org.par$qualifier
...
#   trace<-org.par$trace
# 
#   AFEchidna::echidna.batch(es0.file=es0.file,trait=trait,
#                      fixed=fixed,random=random,residual=residual,
#                      mulT=mulT,mulN=mulN,met=met,mulp=mulp,
#                      delf=delf,trace=trace,maxit=maxit,
#                      predict=predict,vpredict=vpredict,
#                      qualifier=qualifier,jobqualf=jobqualf)
# 
# }
...
# # @export
# repin <- function(object,mulp,signif){
#   UseMethod("repin",object)
# }
# # @method  repin esRb
# # @rdname  pin
# # @export
# repin.esRb<-function(object,mulp,signif=FALSE){ 
#   
#   # object$org.par$batch==T ##??
#   
#   if (!inherits(object, "esRb"))
#     stop("Object must be a batch run result from 'esRb'.\n")
#   
#   repinF(object,mulp,signif)
#   #cat("\n\n")
# }

# @export
# repinF<-function(object,mulp,signif=FALSE){
#   #object<-res22
#   #if(class(object)== "esRb") 
#   data<-object[[1]]
#   
#   nn1<-nn2<-data.frame()
#   NTrait<-names(data)
...
#         x<-AFEchidna::pin11(fm,idx=x,formula=y,digit=digit,all=TRUE,Rres=TRUE)
#       })
#       res<-do.call(rbind,xx)
#       res<-as.data.frame(res)
#       res<-res[!duplicated(res),]
#       res[,-1]<-round(res[,-1],3) # digit=3
#       if(signif==TRUE) res$signif<-AFEchidna::sig.level(res$Estimate,res$SE)
#       res
#     })
#     for(kk in 1:nrow(res)){
#       nn1[i,kk] <- res[kk,2]   # pv
#       nn2[i,kk] <- res[kk,3]   # se
#     }
#   }
#   #colnames(nn1)<-res$Term
#   
#   names(nn2)<-paste0(colnames(nn1),'.se')
#   
#   if(signif==TRUE){
#     nn3<-nn2
#     names(nn3)<-paste0(colnames(nn1),'.sig')
#     for(i in 1:ncol(nn1)) nn3[,i]<-AFEchidna::sig.level(nn1[,i],nn2[,i])
#     nn1<-do.call(cbind,list(nn1,nn2,nn3))
#   }else nn1<-do.call(cbind,list(nn1,nn2))
#   
#   row.names(nn1)<-NTrait
#   
#   terms<-res$Term
#   terms<-paste(paste0('V',1:length(terms)),terms,sep='-')
#   terms<-paste(terms,collapse='; ')
#   
#   cat('\nterms: ',terms,'\n')
#   
#   print.data.frame(nn1)
#   
#   if(signif==TRUE){
#     cat("---------------")
...
# @export
# plot1 <- function(object,idx=NULL,mulT=FALSE,meanN=2.5){
...
# @export
#
# plot1.esRb <- function(object,idx=NULL,mulT=FALSE,meanN=2.5) {
#   #object<-res22
#   mulT<-object$org.par$mulT
# 
#   dat<-lapply(object$res.all,function(x) x$yht)
...
## for multiple esR batch
# @usage 
#       repin22(...,mulp,signif=F)
# @rdname  pin
# @export  repin22
# @export
# repin22<-function(...,mulp,signif=F){
#   args <- list(...)
#   objs <- all.vars(match.call())
#   
#   # lapply(args, function(x) {
#   #   if (!inherits(x, "esRb")){
#   #     stop("Arguments must be of class 'esRb'.")
#   #   }
#   #   invisible(NULL)
#   # })
#   
#   for(i in 1:length(args)){
#     cat("\nrepin results for: ", objs[i],'\n')
#     AFEchidna::repin(args[[i]],mulp=mulp,signif=signif)
#   }
# 
# }
# ------------------------------------
...
#  @export
# mult.pred <- function(file='tmpf0') {
#   txt<-base::readLines(file)
#   
#   a0<-which(grepl('(Predicted values\\s+.*)', txt))
#   heads0<-txt[a0]
#   
...
#   }
#   names(preds0)<-paste0('pred',1:length(aa))
#   preds<-list(heads=heads0,pred=preds0)
#   
#   return(preds)
# }
